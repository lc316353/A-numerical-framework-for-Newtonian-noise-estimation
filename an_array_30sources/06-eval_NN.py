...
def main():
    ...
    freq_x = sig.welch(forces_dipole[mirror], 1/dt, nperseg=nperseg)[0]
    def strainASD(F):
        return np.sqrt(sig.welch(F, 1/dt, nperseg=nperseg)[1]) / (M * 2*pi**2 * np.concatenate(([1], freq_x[1:]))**2 * ET_armlength)
    
    for mirror in range(mirror_count):
        fig, ax = plt.subplots()
        
        ax.plot(freq_x, strainASD(8*pi/3 * G*rho*M * loc_disp[mirror]), label="upper limit", color="darkgrey", linestyle="dotted")
        ax.plot(freq_x, strainASD(forces_dipole[mirror]), label="dipole NN", color="orange")
        ax.plot(freq_x, strainASD(-4*pi/3 * G*rho*M * loc_disp[mirror]), label="lower limit", color="grey", linestyle="dotted")
        
        ax.set_xlabel(r"frequency $f$ [Hz]")
        ax.set_xlim(1, 10)
        ax.set_xscale("log")
        ax.set_xticks([1, 2, 3, 4, 6, 8, 10], [1, 2, 3, 4, 6, 8, 10])
        
        ax.set_ylabel(r"Newtonian noise strain ASD [1/$\sqrt{\mathrm{Hz}}$]")
        ax.set_ylim(2.67e-33 * scaling, 6.67e-30 * scaling)
        ax.set_yscale("log")
    
        ax.legend()
        fig.tight_layout()
        fig.savefig(fig_dir / "NN_strain_ASD.svg")
