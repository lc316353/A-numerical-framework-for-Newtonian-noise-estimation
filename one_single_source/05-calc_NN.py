...
def main():
    ...
    xmin = np.min(mirror_positions[:,0]) - integration_radius
    xmax = np.max(mirror_positions[:,0]) + integration_radius
    ymin = np.min(mirror_positions[:,1]) - integration_radius
    ymax = np.max(mirror_positions[:,1]) + integration_radius
    zmin = np.min(mirror_positions[:,2]) - integration_radius
    zmax = np.max(mirror_positions[:,2]) + integration_radius
    
    inter_x = np.arange(xmin, xmax + dx, dx)
    inter_y = np.arange(ymin, ymax + dx, dx)
    inter_z = np.arange(zmin, zmax + dx, dx)
    XI, YI = np.meshgrid(inter_x, inter_y)

    x3d, y3d, z3d = np.meshgrid(inter_x, inter_y, inter_z)
    ...
    for t in range(from_time_step,to_time_step):
        ...
        # === Interpolate ===
        ordered_x_displacement = griddata((x, y), x_displacement[t], (XI, YI), method="cubic")
        ordered_y_displacement = griddata((x, y), y_displacement[t], (XI, YI), method="cubic")
        ordered_div_displacement = griddata((x, y), div_displacement[t], (XI, YI), method="cubic")
        ordered_curl_displacement = griddata((x, y), curl_displacement[t], (XI, YI), method="cubic")
    
        # === Calculate forces ===
        for mirror in range(mirror_count):
            forces_bulk[mirror][t] = calc_bulk_force(-np.moveaxis(np.stack([ordered_div_displacement]*len(inter_z)),0,-1), mirror)
            forces_dipole[mirror][t] = calc_dipole_force(np.moveaxis(np.stack([ordered_x_displacement]*len(inter_z)),0,-1), np.moveaxis(np.stack([ordered_y_displacement]*len(inter_z)),0,-1), 0, mirror)
